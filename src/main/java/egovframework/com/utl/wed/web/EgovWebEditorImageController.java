...
*/
     ...
     */
    @ResponseBody
    @RequestMapping(value="/utl/wed/insertImage.do")
    public String imageUpload(MultipartHttpServletRequest request,HttpServletResponse response, Model model) throws Exception {
    	...
    	if (list.size() > 0) {
    		...
    		out.println("<script type='text/javascript'>window.parent.CKEDITOR.tools.callFunction('"+funcNum+"','"+url+"','이미지 업로드 완료')</script>");
    		 
    		out.flush();*/
    	}
    	
    	return "{\"uploaded\":\"1\",\"fileName\":\"img1\",\"url\":\""+url+"\"}";
    }
