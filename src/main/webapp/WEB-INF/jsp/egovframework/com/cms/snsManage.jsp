			...
			<ul>
				<li><!-- 조건 -->
	                <select name="searchCondition" id="searchCondition" title="<spring:message code="comUssUmt.userManageSsearch.searchConditioTitle" />"><!--  -->
						...
						<option value="3" <c:if test="${userSearchVO.searchCondition == '3'}">selected="selected"</c:if> >카카오채널</option>
						<option value="4" <c:if test="${userSearchVO.searchCondition == '4'}">selected="selected"</c:if> >블로그</option>
	                </select>
				</li>
				<!-- 검색키워드 및 조회버튼 -->
				<li>
			...
			<c:forEach var="result" items="${resultList}" varStatus="status">
				<tr>
				    ...
				    <td>
						...
						<c:if test="${result.snsType.equals('3')}">
				    		카카오채널
				    	</c:if>
						<c:if test="${result.snsType.equals('4')}">
				    		블로그
				    	</c:if>
